...
COLLISION_THRESHOLD = 0.1 # Abhay Added
    ...
    def step(self, action):
        ...
        if action is not None: # First step without action, called from reset()
            ...
            for car_id, car in enumerate(self.cars):  # Enumerate through cars
                ...
                ### Abhay Modified Code
                if on_grass:
                    step_reward[car_id] -= 1
                
                for car_id_2, car_2 in enumerate(self.cars):  # Enumerate through cars
                    if car_id_2 == car_id:
                        continue

                    car_pos_2 = np.array(car_2.hull.position).reshape((1, 2))
                    distance = np.linalg.norm(car_pos[0]-car_pos_2[0])
                    print(distance, car_width)

                    if distance < car_width:
                        step_reward[car_id] -= 10
